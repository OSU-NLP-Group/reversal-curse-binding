        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    if factored:
                        state["exp_avg_sq_row"] = torch.zeros([grad_shape[-2]]).to(grad)
                        state["exp_avg_sq_col"] = torch.zeros([grad_shape[-1]]).to(grad)
                    else:
                ...
                # mean over all but last two dimensions
                orig_shape = update.shape
                dim_ = len(orig_shape)
                if dim_ > 2:
                    update = update.mean(dim=tuple(range(dim_ - 2)))
                ...
                if factored:
                    ...
                    if dim_ > 2:
                        # expand for broadcasting; momentum shared across dimensions beside last two
                        new_shape = (1,) * (dim_ - 2) + (orig_shape[-2], orig_shape[-1])
...
#                         state["exp_avg"] = torch.zeros_like(grad)
#                     if factored:
#                         state["exp_avg_sq_row"] = torch.zeros(grad_shape[:-1]).to(grad)         # [bsz, 4]
#                         state["exp_avg_sq_col"] = torch.zeros(grad_shape[:-2] + grad_shape[-1:]).to(grad)   # [bsz, 768]
#                     else:
#                         state["exp_avg_sq"] = torch.zeros_like(grad)
